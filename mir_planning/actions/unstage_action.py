def perform_unstage(instance, action):
    ...
    if not user_input or user_input == '' or user_input == ' ' :
        user_input = 'y'
    if user_input == 'y':
        instance.problem.set_initial_value(instance.fluents_dict['stored'](instance.objects_dict[object], instance.objects_dict[platform]), False)
        ...
        instance.get_logger().warn(f"Unstage failed for {object} but nothing changed in KB")
        return False